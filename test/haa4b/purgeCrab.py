...
class UserCacheHelper:
    def __init__(self, proxy=None, user=None):
        ...
        if proxy is None:
            files = [os.path.join('/tmp',name) for name in os.listdir('/tmp/') if os.path.isfile(os.path.join('/tmp',name)) and name.startswith('x509up')]
            for f in files:
                if getpwuid(os.stat(f).st_uid).pw_name == self.user:
                    self.proxy = f
                    break
        if self.proxy is None:
          raise Crab3ToolsException('Can not find grid proxy, get grid proxy first')
...
if __name__ == '__main__':
    h = UserCacheHelper()
    print 'CRAB cache quota: ', h.quota()
    for x in h.filelist():
        if '.log' in x:
            continue
        print 'remove', x
