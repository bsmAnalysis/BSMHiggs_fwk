...
<<EOF
    ...
    result1=`grep "Unable to determine uncertainties on all fit parameters" out_60/log-first.txt`
    result2=`grep "Unable to determine uncertainties on all fit parameters" out_60/log.txt` 
    echo -e "$result1 \n" >> $tempfile1
    echo -e "$result2 \n" >> $tempfile2   
...
EOF
    ...
    cp impacts/impacts.pdf ../impacts_${ch}_${year}.pdf 
    # make GOF plot:
