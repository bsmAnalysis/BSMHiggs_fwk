...
inUrl='$CMSSW_BASE/src/UserCode/bsmhiggs_fwk/test/haa4b/plotter_2018_05_11_forLimits.root' #plotter_2017_05_05_forLimits.root'
...
#           SCRIPT.writelines("computeLimit --m " + str(m) + " --in " + inUrl + " " + "--syst --blind --index 1 --bins " + BIN[iConf] + " --json " + jsonUrl + " " + SideMassesArgs + LandSArg + cutStr  +" ;\n")
...
#           SCRIPT.writelines("combine -M FitDiagnostics workspace.root --plots --saveNormalizations --saveShapes --saveWithUncertainties --saveNLL --rMin=-2 --rMax=2 --robustFit 1 > log.txt \n")
           # -t -1 --expectSignal=1
           SCRIPT.writelines("combine -M FitDiagnostics workspace.root -m " +  str(m) + " --plots --saveNormalizations --saveShapes --saveWithUncertainties --saveNLL --stepSize=0.05 --rMin=-5 --rMax=5 --robustFit 1 --setParameters mask_E_SR_qcdB_Mt_3b=1,mask_MU_SR_qcdB_Mt_3b=1,mask_E_SR_qcdB_Mt_4b=1,mask_MU_SR_qcdB_Mt_4b=1 > log.txt \n") 
...
#              SCRIPT.writelines("combine -M Asymptotic -m " +  str(m) + " workspace.root --run blind --setParameters tt_norm_e=$tt_e,tt_norm_mu=$tt_mu,w_norm_3b_e=$w_3b_e,w_norm_3b_mu=$w_3b_mu,w_norm_4b_e=$w_4b_e,w_norm_4b_mu=$w_4b_mu >  COMB.log;\n")       
              SCRIPT.writelines("combine -M Asymptotic -m " +  str(m) + " workspace.root -t -1 --setParameters tt_norm_e=$tt_e,tt_norm_mu=$tt_mu,w_norm_3b_e=$w_3b_e,w_norm_3b_mu=$w_3b_mu,w_norm_4b_e=$w_4b_e,w_norm_4b_mu=$w_4b_mu > COMB.log;\n")  
