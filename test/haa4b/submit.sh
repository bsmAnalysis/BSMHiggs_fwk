...
if [[ $step > 0.999 &&  $step < 2 ]]; then
   ...
   if [[ $step == 1.1 ]]; then  #submit jobs for h->aa->XXYY analysis
	...
	# @btagSFMethod=2: Event reweighting using discriminator-dependent scale factors
       runLocalAnalysisOverSamples.py -e runhaaAnalysis -b $BTAG_NTPL_OUTDIR -g $RUNLOG -j $NTPL_JSON -o $NTPL_OUTDIR -d $NTPL_INPUT -c $RUNNTPLANALYSISCFG -p "@runSystematics=True @runMVA=False @reweightDYZPt=False @reweightTopPt=True @usemetNoHF=False @verbose=False @useDeepCSV=True @runQCD=False @runZH=$doZH @btagSFMethod=1" -s $queue -t MC13TeV_TTJets # -r true
# MC13TeV_TTTo  #-r True
   fi
fi
