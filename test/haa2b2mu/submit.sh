...
## Run 2l2vAnalysis over samples
    ...
    printf "\n\t%-5s  %-40s\n"  "1"  "run 'runhaaAnalysis' on all samples" 
    printf "\n\t%-5s  %-40s\n"  "1.0"  "make plots for mcbased analysis"
    printf "\n\t%-5s  %-40s\n"  "1.1"  "run 'runhaaAnalysis' on photon samples" 
    printf "\n\t%-5s  %-40s\n"  "1.2"  "run 'runhaaAnalysis' on photon samples with photon re-weighting"

## Run photonZ closure tests:
    printf "\n\t%-5s  %-40s\n"  "1.3"  "run 'runhaaAnalysis' on photon and DY samples in MC only"
    printf "\n\t%-5s  %-40s\n"  "1.4"  "run 'runhaaAnalysis' on photon samples with photon re-weighting with MC weights"  
...
if [[ $step > 0.999 &&  $step < 2 ]]; then
   if [[ $step == 1 || $step == 1.0 ]]; then        #submit jobs for 2l2v analysis
	echo "JOB SUBMISSION"
	runAnalysisOverSamples.py -e runhaaAnalysis -j $JSON -o $RESULTSDIR  -c $MAINDIR/../runAnalysis_cfg.py.templ -p "@data_pileup=datapileup_latest @jacknife=0 @saveSummaryTree=True @runSystematics=False @useMVA=True @jacks=0" -s $queue --report True --key 2l2v_mcbased $arguments
   fi

   if [[ $step == 1 || $step == 1.1 ]]; then        #submit jobs for 2l2v photon jet analysis
	echo "JOB SUBMISSION for Photon + Jet analysis"
	runAnalysisOverSamples.py -e runhaaAnalysis -j $JSON -o $RESULTSDIR -c $MAINDIR/../runAnalysis_cfg.py.templ -p "@data_pileup=datapileup_latest @jacknife=0 @saveSummaryTree=True @runSystematics=False @useMVA=True @jacks=0"  -s $queue --report True --key 2l2v_photoncontrol --skipkey 2l2v_mcbased $arguments 
   fi
   ...
   if [[ $step == 1.3 ]]; then        #submit jobs for 2l2v analysis with photon re-weighting in MC 
       ...
       runAnalysisOverSamples.py -e runhaaAnalysis -j $JSON -o $RESULTSDIR -c $MAINDIR/../runAnalysis_cfg.py.templ -p "@data_pileup=datapileup_latest @useMVA=True @saveSummaryTree=True @runSystematics=False @automaticSwitch=False @is2011=False @jacknife=0 @jacks=0" -s $queue --report True --key 2l2v_photonZ $arguments
   fi
