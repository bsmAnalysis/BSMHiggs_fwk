...
jsonPath='$CMSSW_BASE/src/UserCode/bsmhiggs_fwk/test/haa4b/samples_full2016_GGH_VBF.json' #samples_full2016_GGH_WithoutBckg.json' #samples_full2016_GGH_WithoutWZ.json' #samples_full2016_GGH_WithoutWZandZVV.json' #samples_full2016_GGH_WithoutIrriducible.json' #samples_full2016_GGH.json'
inUrl='$CMSSW_BASE/src/UserCode/bsmhiggs_fwk/test/haa4b/plotter_2017_05_12_forLimits.root' #plotter_2017_05_05_forLimits.root'
...
for signalSuffix in signalSuffixVec : 
   ...
   #                     Gcut[c-1].SetPoint(mi, float(vals[0]), float(vals[c+1]));
	   ...
	   #SCRIPT.writelines("computeLimit --m " + str(m) + " --in " + inUrl + " " + " --index " + indexString + " --bins " + BIN[iConf] + " --json " + jsonUrl + " " + SideMassesArgs + " " + LandSArg + cutStr  +" ;\n")
           ...
           SCRIPT.writelines("sh combineCards.sh;\n"); 
	   SCRIPT.writelines("text2workspace.py card_combined.dat -o workspace.root -P UserCode.bsmhiggs_fwk.HiggsWidth:higgswidth --PO verbose --PO \'is2l2nu\' --PO m=\'" + str(m) + "\' --PO w=\'" + str(cp) + "\' \n")
           #compute pvalue
