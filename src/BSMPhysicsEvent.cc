...
PhysicsEvent_t getPhysicsEventFrom(DataEvtSummary_t &ev)
{
    ...
    for(Int_t i=0; i<ev.mn; i++) {
        ...
        if(P4.pt()>0) {
            ...
            phys.leptons[nlep].setLeptonIsoInfo(ev.mn_pileupIsoR03[i],ev.mn_chargedIsoR03[i],ev.mn_photonIsoR03[i],ev.mn_neutralHadIsoR03[i],ev.mn_passIso[i],
	...
	//        if(ev.mc_status[ipart]!=1) continue;
	  ...
	  phys.genneutrinos.push_back( PhysicsObject(p4,ev.mc_id[ipart],ev.mc_mom[ipart],ev.mc_momidx[ipart]) );
        ...
        case 2000012:
        case -2000012: {
	  phys.genWIMPs.push_back( PhysicsObject(p4,ev.mc_id[ipart],ev.mc_mom[ipart],ev.mc_momidx[ipart]) );
        }
        break;
	case 35:
        case 36: {
