...
void mainNtuplizer::analyze(const edm::Event& event, const edm::EventSetup& iSetup)  //edm::Event const& event, edm::EventSetup const& iSetup) //
{
  ...
  for (const pat::Jet &j : fatjets) {
    if ( verbose_ ) printf("      AK8 jets : %d/%lu | pt=%.3f | eta=%.3f | pFlavour=%d | hFlavour=%d | nSubjets=%lu \n",
      ifjet,
      fatjets.size(),
      j.pt(),
      j.eta(),
      j.partonFlavour(),
      j.hadronFlavour(),
      j.subjets("SoftDropPuppi").size()
    );
    auto const & sdSubjets = j.subjets("SoftDropPuppi");
    int nSub(sdSubjets.size());

    if(nSub > 0){
      if ( verbose_ ) printf("            Subjet 0 : pt=%.3f | eta=%.3f | pFlavour=%d | hFlavour=%d \n",
        (sdSubjets[0])->pt(),
        (sdSubjets[0])->eta(),
        (sdSubjets[0])->partonFlavour(),
        (sdSubjets[0])->hadronFlavour()
      );
    }
    if(nSub > 1){
      if ( verbose_ ) printf("            Subjet 1 : pt=%.3f | eta=%.3f | pFlavour=%d | hFlavour=%d \n",
        (sdSubjets[1])->pt(),
        (sdSubjets[1])->eta(),
        (sdSubjets[1])->partonFlavour(),
        (sdSubjets[1])->hadronFlavour()
      );
    }
...
//           ev.fjet_elf[ev.fjet] = j.electronEnergyFraction(); //.chargedEmEnergyFraction();
//           // //------
//           // ev.fjet_ecfB1N2[ev.fjet] = (float) j.userFloat("ak8PFJetsPuppiSoftDropValueMap:nb1AK8PuppiSoftDropN2");
//           // ev.fjet_ecfB1N3[ev.fjet] = (float) j.userFloat("ak8PFJetsPuppiSoftDropValueMap:nb1AK8PuppiSoftDropN3");
...
//           }
//           if(nSub >0 ){
//             ev.fjet_subjets_btag[ev.fjet][0] = (j.subjets("SoftDrop"))[0]->bDiscriminator("pfDeepCSVJetTags:probb") + (j.subjets("SoftDrop"))[0]->bDiscriminator("pfDeepCSVJetTags:probbb");
...
//           }
//           if(nSub >1 ){
//             ev.fjet_subjets_btag[ev.fjet][1] = (j.subjets("SoftDrop"))[1]->bDiscriminator("pfDeepCSVJetTags:probb") + (j.subjets("SoftDrop"))[1]->bDiscriminator("pfDeepCSVJetTags:probbb");
